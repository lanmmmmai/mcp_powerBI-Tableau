...
if sms_start == -1:
    print("ERROR: page-section-sms not found")
    exit(1)

curr = sms_start
replacements = [NEW_SVG_1, NEW_SVG_2, NEW_SVG_3, NEW_SVG_4, NEW_SVG_5]
...
for i, new_svg in enumerate(replacements, 1):
    ...
    print(f"Replacing SVG #{i}: old {len(old_svg)} -> new {len(new_svg)}")
    text = text[:svg_start] + new_svg + text[svg_end:]
    curr = svg_start + len(new_svg)

if ".visual-chart-svg {" not in text:
    css_insert = """
    .visual-chart-svg {
      width: 100%;
      height: auto;
...
"""
    ...
    text = text[:style_end] + css_insert + text[style_end:]
    print("Added .visual-chart-svg CSS rule.")
